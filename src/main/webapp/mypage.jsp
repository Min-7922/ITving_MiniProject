...
<main class="frame">
	...
	<div>
		<div class="mypage-bt flex">
			...
			<div class="${param.list == 'like' ? 'bgcolor1' : 'bgcolor2'}">
				<a href="${cpath }/mypage.jsp?list=like">
					<button>좋아요 한 프로그램</button>
				</a>
			</div>
		</div>
	...
	<c:if test="${param.list == 'like'}">
		<div class="mypage-program">
			<div class="flex wrap program-list-img">
				<c:set var="recommendlist" value="${recommendDAO.selectProgramByWriter(login.userid) }" />
				<c:forEach var="recommenddto" items="${recommendlist }">
					<c:set var="programdto3" value="${programDAO.selectOne(recommenddto.category_idx) }" />
					<div><a href="${cpath }/board.jsp?idx=${programdto3.idx}">
						<img src="${cpath }/programImage/${programdto3.image}"
